...
std::array<int, 3> deblocking(const EncodePicture &p) {
...
void validate(const EncodeSettings &s, const EncodePicture &p, unsigned width, unsigned height) {
    ...
    const auto &slice = p.slices.front();
    check((slice.slice_type % 5 == 2) == bool(pic.pic_fields.bits.idr_pic_flag),
          "only IDR and P pictures are supported", VA_STATUS_ERROR_UNIMPLEMENTED);
    ...
    const auto filter = deblocking(p);
...
struct Encoder::Impl {
    ...
    void configure_deblocking() {
        const unsigned ids[] = {V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_MODE,
        ...
        if (!supported) {
            ...
            check(filter == std::array<int, 3>{0, 0, 0},
                  "H264 deblocking request requires Iris loop-filter controls",
                  VA_STATUS_ERROR_UNIMPLEMENTED);
            return;
        }
...
Encoder::Encoder(const std::string &device, unsigned width, unsigned height, VAProfile profile,
    ...
    auto impl =
        std::make_unique<Impl>(width, height, s, p.params.pic_fields.bits.entropy_coding_mode_flag);
    auto &e = *impl;
    e.filter = deblocking(p);
    ...
    checked(e.fd, VIDIOC_S_FMT, &fmt, "encoder S_FMT H264");
    fmt = {};
    fmt.type = OUTPUT;
    fmt.fmt.pix_mp.width = width;
    fmt.fmt.pix_mp.height = height;
    ...
    e.control(V4L2_CID_MPEG_VIDEO_H264_PROFILE,
              ...
              : profile == VAProfileH264Main ? V4L2_MPEG_VIDEO_H264_PROFILE_MAIN
    ...
    e.control(V4L2_CID_MPEG_VIDEO_H264_MIN_QP, s.min_qp);
    e.control(V4L2_CID_MPEG_VIDEO_H264_MAX_QP, s.max_qp);
...
std::shared_ptr<EncodeTask> Encoder::encode(const EncodeSettings &s, const EncodePicture &p,
    ...
    check(deblocking(p) == e.filter, "changing H264 deblocking requires a new context",
          VA_STATUS_ERROR_UNIMPLEMENTED);
    check(s.sequence.level_idc == e.settings.sequence.level_idc &&
    ...
    const bool intra = p.slices.front().slice_type % 5 == 2;
    check(e.token || p.params.pic_fields.bits.idr_pic_flag, "encoder must start with IDR",
          VA_STATUS_ERROR_INVALID_PARAMETER);
    if (!intra)
        check(p.slices.front().RefPicList0[0].picture_id == e.last_reference &&
                  !(p.slices.front().RefPicList0[0].flags &
                    (VA_PICTURE_H264_INVALID | VA_PICTURE_H264_LONG_TERM_REFERENCE)) &&
                  e.last_reference != VA_INVALID_SURFACE,
              "encoder only supports the immediately preceding reference",
              VA_STATUS_ERROR_UNIMPLEMENTED);
    check(p.params.pic_fields.bits.reference_pic_flag, "non-reference encoding unsupported",
          VA_STATUS_ERROR_UNIMPLEMENTED);
    ...
    int qp = int(p.params.pic_init_qp) + p.slices.front().slice_qp_delta;
    ...
    try {
        ...
        if (s.rate_control == VA_RC_CQP && qp != e.current_qp) {
            e.control(V4L2_CID_MPEG_VIDEO_H264_I_FRAME_QP, qp);
            e.control(V4L2_CID_MPEG_VIDEO_H264_P_FRAME_QP, qp);
            e.current_qp = qp;
        }
        if (intra && e.token)
        ...
        e.token = token;
        e.last_reference = p.params.CurrPic.picture_id;
